...
$AuditScriptBlock = {
    ...
    try {
        ...
        # ── SP2013 Workflows ──────────────────────────────────────────────────
        try {
            # Fix: WorkflowServicesManager is not a ClientObject — do NOT ctx.Load() it.
            # GetWorkflowSubscriptionService / GetWorkflowDefinitionService are local calls
            # that return proxy objects; no ExecuteQuery needed here.
            ...
            try {
                ...
                if ($xsnFiles.Count -eq 0) {
                    $IpResults.Add([PSCustomObject]@{
                        ...
                        FormUrl  = if ($list.DefaultViewUrl) { $webUrl.TrimEnd('/') + '/' + $list.DefaultViewUrl.TrimStart('/') } else { $webUrl }
